...
plot.maccest <- function(x, main = NULL, xlab = NULL, ylab = NULL, ...)
{
  ...
  plot(x$acc, type = "o", main=main,xlab=xlab, ylab=ylab, col="blue",
       ylim=ylim, xaxt = "n",...)
...
mbinest <- function(dat, cl, choices = NULL, method, pars=valipars(),...)
{
  ...
  ret <- list(all=res, com=dat.bin$com, acc=acc, auc=auc,mar=mar,method=method,
...
mc.norm <- function(x,...)
{
  ...
  X11()
  print(histogram(~ values|ind, data=x, as.table=T,
