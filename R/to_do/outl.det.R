...
## ==========================================================================
## lwc-30-01-2013: Multivariate outlier detection
## Note: 1.) check out.det in package FIEispro
...
## ==========================================================================
## lwc-23-01-2013: outlier detection based on boxplot. use Boxplot in package
##                 car
...
aq.plot.1 <-function(x, conf.level = 0.975, plotting = TRUE,...)
{
  ...
  if (plotting) {
    ...
    X11(width=8, height=5); par(mfrow=c(1, 2))
    ## -----------
    ...
    text(x=delta + xrange/20, y=0.4, paste(100*(pchisq(delta,df=ncol(x))),"% Quantile",sep=""),
