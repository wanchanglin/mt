...
feat.cons <- function(freq, disp=TRUE) {
  ...
  mat <- matrix(data=NA, nrow=length(fs.names),ncol=length(freq),
...
feat.mfs <- function(x,y, method, pars=valipars(),is.resam=TRUE,...) {
  ...
  fs.stats <- if (is.resam) {
              ...
              } else {sapply(res,function(x) x$stats)}

  feat.res <- list(fs.order=fs.order, fs.rank=fs.rank, fs.stats=fs.stats,
                   all=res)
...
feat.mfs.stab <- function(fs.res,rank.cutoff=20,freq.cutoff=0.5){
  ...
  fs <- list(fs.freq = fs.freq, fs.subs = fs.subs,
...
feat.mfs.stats <- function(fs.stats,cumu.plot=FALSE, main="Stats Plot",
  ...
  st.p <- xyplot(value~idx|variable, data=st,
...
feat.rank.re <- function(x,y,method,pars=valipars(),tr.idx=NULL,...) {
  ...
  res <- list(method     = method,
              ...
              all        = res.all)        #' all results of re-sampling
  return(res)
}
...
fs.rf.1 <- function(x,y,fs.len="power2",...) {
  ...
  for( i in 1:nlen ) {
    ...
    sel <- fs.order[1:len[i]]
    #' call randomForest
    rf   <- randomForest(x[,sel,drop=F],y,importance=T,
                         keep.forest = FALSE,...)
...
#' =========================================================================
frankvali.default <-
  function(dat,cl,cl.method = "svm", fs.method="fs.auc",
           ...
           tr.idx=NULL,...)
{
  ...
  for (i in 1:pars$niter) {
    ...
    for (j in 1:length(train.ind)) {
      ...
      #' method.
      res[[j]] <- frank.err(dat.tr, cl.tr, dat.te, cl.te, cl.method=cl.method,
...
frank.err <- function(dat.tr, cl.tr, dat.te, cl.te, cl.method="svm",
  ...
  for( i in 1:nlen ) {
    ...
    error[i] <- classifier(dat.tr[,sel, drop=F],cl.tr,dat.te[,sel,drop=F],cl.te,
                           method=cl.method, ...)$err
  }

  ret <- list(cl.method = cl.method, fs.len=len, error=error,
...
if (F) {
  ...
  fs  <- fs.rf(x.1, y.1)
  ord <- fs$fs.order[1:50]
  res.1 <- fs.cl.2(x.1,y.1,fs.order=ord, cl.method = "svm", pars = pars,
                   agg_f=TRUE)
  perf.aam(res.1)
}
#' -------------------------------------------------------------------------
